...
Junyi Guan, Member, IEEE , Bingbing Jiang , Member, IEEE , Weiguo Sheng , Member, IEEE , Yangyang Zhao , Sheng Li , Senior Member, IEEE , and Xiongxiong He
...
1 The code is available at https: // github.com / Guanjunyi / PeakPading.

Fig. 2. (a) DPC-padding strategy's center detection, (b) min-padding strategy's center detection, and (c) human perception of peak-padding.

<!-- image -->

Fig. 3. Limitation of the min-padding strategy on Euclidean space.

<!-- image -->

and eliminate noise, its parameter tuning is often tedious. Later works, e.g., [27], developed the adaptive parameter tuning, still, they would possibly merge high-overlapping clusters [28]. In PeakPad, the centers of high-overlapping clusters can be detected to avoid merging. Inspired by DBSCAN, points within a cluster are ensured to be density-connected and the concept of 'DC distance' is introduced to quantify the density-connectivity between points to achieve the robust reconstruction of complex-shaped clusters.

Guan et al. [13] argued that being a 'density peak' is a necessary but not su GLYPH&lt;14&gt; cient condition for identifying cluster centers. To address this, they introduced the 'peak graph,' a sparse graph structure where nodes represent density peaks. This approach not only aids in selecting appropriate centers but also significantly reduces computational complexity without sacrificing accuracy. Cheng et al. [29] use natural density peaks (NDPs) combined with neighbor-based graph distance (GD) to perform clustering via K -means [47], allowing detection of arbitrarily shaped clusters. Each NDP represents a single-peak cluster, containing all associated data points and having the highest density. In PeakPad, we first detect singlepeak clusters represented by local density peaks in the dataset. Unlike the NDP-K-means method [29], we utilize the peak graph structure and design a min-padding strategy on it to obtain the final multipeak clusters.
